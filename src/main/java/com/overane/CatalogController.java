...
public class CatalogController {
    ...
    @Autowired
    private CartEntryRepositori cartEntryRepositori;
    ...
    public String showCatalogPage(final Model model) {
        ...
        model.addAttribute("user", clientShopRepository.findByEmail(currentPrincipalName));
        model.addAttribute("product", productRepository.findAll());
        model.addAttribute("cart", clientShopRepository.findByEmail(currentPrincipalName).getCart());
        return "catalog";
    ...
    public String addToCart(@RequestParam(name = "product") final Long productId, final Model model) {
        ...
        if (product.isPresent() && clientShopRepository.findByEmail(currentPrincipalName).getCart() == null) {
            clientShopRepository.save(clientShopRepository.findByEmail(currentPrincipalName));
            Cart cart = new Cart();
            ...
            cartEntryRepositori.save(cartEntry);
            cart.getCartEntries().add(cartEntry);
            cartRepositori.save(cart);
            clientShopRepository.findByEmail(currentPrincipalName).setCart(cart);
        } else if (product.isPresent()) {
            boolean isFound = false;
            final Cart cart = clientShopRepository.findByEmail(currentPrincipalName).getCart();
        ...
        if (product.isPresent()) {
            for (CartEntry c : cart.getCartEntries()) {
                if (c.getProduct().getId() == product.get().getId()) {
                    cart.getCartEntries().remove(c);
                    cartRepositori.save(cart);
                    cartEntryRepositori.delete(c);
                    break;
                }
            }
        }
